 ...
 * @api public
 */
Client.prototype.publish = function (subject, msg, reply, callback) {
  ...
  if (!subject) {
    if (callback) {
      callback(new NatsError(BAD_SUBJECT_MSG, BAD_SUBJECT))
    } else {
  ...
  if (typeof msg === 'function') {
    if (callback || reply) {
      callback(new NatsError(BAD_MSG_MSG, BAD_MSG))
      return
    }
    // noinspection JSValidateTypes
    callback = msg
  ...
  if (typeof reply === 'function') {
    if (callback) {
      callback(new NatsError(BAD_REPLY_MSG, BAD_REPLY))
      return
    }
    // noinspection JSValidateTypes
    callback = reply
